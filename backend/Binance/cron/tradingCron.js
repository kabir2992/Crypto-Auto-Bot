...
const { executeBuy, executeSell, closeShortPositions, executeShortSell } = require("../services/tradeService");
...
cron.schedule("*/5 * * * *", async () => {
  ...
  try {
    ...
    // const hasShorts = botState.shortPositions?.length > 0;
    // const hasLongs = botState.positions?.length > 0;
    ...
    if (action === "LONG_BUY" || action === "long_buy" || action === "BUY") {
    ...
    if (action === "LONG_SELL" || action === "long_sell") {
    ...
    if (action === "SHORT_CLOSE" || action === "short_close") {
      botState.botMode = "SHORT CLOSING";
      // console.log( "BUY SIGNAL DETECTED" );
      // botState.currentStrategy = "Trend Reversal Buy (Bullish)";
      await botState.save();
      await closeShortPositions(currentPrice);
    }
    ...
    if (action === "SHORT_SELL" || action === "short_sell" || action === "SELL") {
      botState.botMode = "SHORT SELLING";
      // console.log( "TAKE PROFIT SELL" );
      // botState.currentStrategy = "High RSI with Min. Profit (Bearish)";
      await botState.save();
      await executeShortSell(currentPrice);
    }
